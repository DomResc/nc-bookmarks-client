...
interface AddBookmarkModalProps {
  folders: Folder[];
  onClose: () => void;
  onSave: (data: { title: string; url: string; tags: string; folders: string }) => Promise<void>;
  onCreateFolder: (title: string, parentFolderId: number) => Promise<number>;
}

export default function AddBookmarkModal({ folders, onClose, onSave, onCreateFolder }: AddBookmarkModalProps) {
  ...
  async function handleSave() {
    ...
    setError(null);
    try {
      let folderValue = selectedFolderId;
      if (useCustomFolder) {
        if (!customFolder.trim()) { setError('Folder name is required'); setSaving(false); return; }
        const newId = await onCreateFolder(customFolder.trim(), newFolderParentId ? Number(newFolderParentId) : -1);
        folderValue = String(newId);
      }
      await onSave({ title: title.trim(), url: url.trim(), tags, folders: folderValue });
      setSaving(false);
      onClose();
    } catch (err: unknown) {
    ...
    <div className="absolute inset-0 bg-black/40 dark:bg-black/60 flex items-end z-50">
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h2 id="add-bookmark-title" className="text-base font-semibold text-gray-900 dark:text-gray-100">
            New bookmark
          </h2>
          <button
            onClick={onClose}
        ...
        {loading ? (
          <div className="flex items-center justify-center py-8">
            <Spinner size={24} trackClassName="text-gray-200 dark:text-gray-700" indicatorClassName="text-blue-500" />
          </div>
        ) : (
          <div className="space-y-3">
            <div>
              <label htmlFor="bm-title" className="sr-only">Title</label>
              <input
                id="bm-title"
                type="text"
                value={title}
            ...
            <div>
              <label htmlFor="bm-url" className="sr-only">URL</label>
              <input
                id="bm-url"
                type="url"
                value={url}
            ...
            <div>
              <label htmlFor="bm-tags" className="sr-only">Tags (comma-separated)</label>
              <input
                id="bm-tags"
                type="text"
                value={tags}
            ...
            <div>
              <label htmlFor="bm-folder-select" className="sr-only">Folder</label>
              {!useCustomFolder ? (
                ...
                <div className="space-y-2">
                  ...
                  <div>
                    <label htmlFor="bm-new-folder-parent" className="sr-only">Parent folder</label>
                    <select
                      id="bm-new-folder-parent"
            ...
            {error && (
              <div className="p-2.5 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg text-red-700 dark:text-red-300 text-xs">
                {error}
              </div>
            )}
            ...
            <div className="flex gap-2 pt-1">
              <button
                onClick={onClose}
                disabled={saving}
                className="flex-1 py-2.5 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg text-sm font-medium hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors disabled:opacity-50"
              >
                Cancel
              </button>
              <button
